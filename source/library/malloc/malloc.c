...
static char heap[SZ_128M] __attribute__((__used__, __section__(".heap")));
static void * __pool;
...
{
	__pool = memory_pool_create((void *)heap, sizeof(heap));
}

void malloc_exit(void)
{
	memory_pool_destroy(__pool);
}

void * malloc(size_t size)
{
	return tlsf_malloc(__pool, size);
}

void * memalign(size_t align, size_t size)
{
	return tlsf_memalign(__pool, align, size);
}

void * realloc(void * ptr, size_t size)
{
	return tlsf_realloc(__pool, ptr, size);
}
